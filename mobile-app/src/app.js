...
function frozenOrderMoney(item,value,sourceCurrency=''){
  ...
  if(!invoice||!fx||!Number.isFinite(n))return money(value,sourceCurrency);
  const target=String(invoice.currency||fx.currency||'').toUpperCase();if(!target)return money(value,sourceCurrency);
  let rate=0;
  if(String(fx.sourceCurrency||'').toUpperCase()===source)rate=Number(fx.rate);
  else{
    const src=activeCurrencies().find(x=>x.code===source),targetRate=Number(fx.targetRate);
    if(src&&Number(src.rate)>0&&targetRate>0)rate=targetRate/Number(src.rate);
  }
  return rate>0?exactMoney(n*rate,target):money(value,sourceCurrency);
}
...
function renderHome(){
  ...
  if(role==='client'){
    ...
    $('screen').innerHTML=pageHeader(`${tr('مرحبًا','Welcome')} ${esc(currentUser.name||currentUser.company||'')}`,tr('ركز على الطلبات التي تحتاج إجراء منك أولًا.','Focus first on the items that need your action.'))+
      ...
      `<div class="stats-grid supplier-home-stats supplier-three-stats">${statCard(activeOrders.length,tr('الطلبات النشطة','Active orders'),'orders')}${statCard(invites.length,tr('طلبات عروض الأسعار','Quote requests'),'requests')}${statCard(publishedPublic,tr('مصادر التوريد المعتمدة','Approved supply offers'),'view-public-offers')}</div>`+
...
function selectedQuotePanel(r){
  ...
  return `<section class="client-selected-quote"><div class="client-detail-section-head"><div><small>${esc(tr('العرض المختار','Selected quote'))}</small><strong>#${esc(ref(q))}</strong></div><span class="status-pill status-published">${esc(tr('مختار','Selected'))}</span></div><div class="client-selected-values"><div><span>${esc(tr('سعر الوحدة','Unit price'))}</span><strong>${frozenOrderMoney(r,q.unitPrice,q.currency)}</strong></div><div><span>${esc(tr('الكمية','Quantity'))}</span><strong>${esc(r.quantity||'—')}</strong></div><div class="total"><span>${esc(tr('الإجمالي','Total'))}</span><strong>${total!==null?frozenOrderMoney(r,total,q.currency):'—'}</strong></div></div></section>`;
}
function clientCartReplacementPanel(r){
  const pending=r?.pendingCartReplacement;if(!pending?.quoteId||!pending?.interestId)return '';
  const line=(Array.isArray(r.cartItems)?r.cartItems:[]).find(x=>x.interestId===pending.interestId)||{};
  const diff=Number(pending.difference||0),currency=pending.currency||line.currency||r.currency||'SAR';
  const diffText=Math.abs(diff)<0.000001?tr('بدون تغيير في الإجمالي','No change to total'):diff>0?tr('زيادة ','Increase ')+frozenOrderMoney(r,diff,currency):tr('تخفيض ','Reduction ')+frozenOrderMoney(r,Math.abs(diff),currency);
  return `<section class="client-detail-action action cart-replacement-approval"><div><small>${esc(tr('موافقتك مطلوبة','Your approval is required'))}</small><strong>${esc(tr('المورد السابق تعذر عليه التنفيذ ويوجد سعر بديل لهذا المنتج.','The previous supplier could not fulfill this item and a replacement price is available.'))}</strong><p>${esc(snapshotTitle(line))}</p><div class="facts"><span>${esc(tr('السعر السابق','Previous price'))}: ${frozenOrderMoney(r,pending.oldUnitPrice,currency)}</span><span>${esc(tr('السعر الجديد','New price'))}: ${frozenOrderMoney(r,pending.newUnitPrice,currency)}</span><span>${esc(diffText)}</span><span>MOQ ${esc(pending.moq||'—')}</span><span>${esc(tr('مدة التنفيذ','Lead time'))}: ${esc(pending.leadTime||'—')}</span></div></div><div class="client-detail-actions"><button type="button" class="primary-small" data-approve-cart-replacement="${esc(pending.quoteId)}" data-request-id="${esc(r.id)}">${esc(tr('الموافقة على السعر والعرض الجديد','Approve new price & quote'))}</button><button type="button" class="secondary-btn" data-reject-cart-replacement="${esc(pending.quoteId)}" data-request-id="${esc(r.id)}">${esc(tr('رفض العرض وطلب مورد آخر','Reject & request another supplier'))}</button></div></section>`;
}
function clientRequestActionPanel(r){
  if(r.pendingReplacementQuoteId){
    ...
    if(q)return `<section class="client-detail-action action"><div><small>${esc(tr('موافقتك مطلوبة','Your approval is required'))}</small><strong>${esc(tr('المورد السابق تعذر عليه التنفيذ ويوجد عرض بديل بشروط مختلفة.','The previous supplier could not fulfill the order and a replacement quote has different terms.'))}</strong><p>${esc(tr('السعر','Price'))}: ${frozenOrderMoney(r,q.unitPrice,q.currency)} · MOQ ${esc(q.moq||'—')} · ${esc(tr('مدة الإنتاج','Production time'))}: ${esc(q.leadTime||'—')}</p></div><div class="client-detail-actions"><button type="button" class="primary-small" data-approve-replacement-quote="${esc(q.id)}" data-request-id="${esc(r.id)}">${esc(tr('الموافقة على العرض البديل','Approve replacement quote'))}</button><button type="button" class="secondary-btn" data-reject-replacement-quote="${esc(q.id)}" data-request-id="${esc(r.id)}">${esc(tr('رفض العرض وطلب مورد آخر','Reject & request another supplier'))}</button></div></section>`;
  }
...
function quoteFormHtml(q=null,requestId=''){
  const rid=requestId||q?.requestId||'',request=(platformState?.requests||[]).find(r=>r.id===rid),replacementCurrency=request?.orderType==='cart_replacement'?String(request.replacementCurrency||'').toUpperCase():'';
  const currencies=replacementCurrency?[replacementCurrency]:['USD','SAR','AED','CNY','EUR'],moqMax=request?.orderType==='cart_replacement'&&Number(request.quantity)>0?Number(request.quantity):'';
  const replacementNote=replacementCurrency?`<p class="form-hint">${esc(tr('هذا تسعير بديل لمنتج داخل طلب قائم. العملة ثابتة حسب الطلب الأصلي ولا يظهر لك سعر المورد السابق.','This is replacement pricing for an item in an existing order. Currency is fixed to the original order, and the previous supplier price is not shown.'))}</p>`:'';
  return `<form id="quoteForm" class="form-stack" data-id="${esc(q?.id||'')}" data-request="${esc(rid)}">${replacementNote}<div class="form-two"><label><span>${esc(t('price'))}</span><input name="unitPrice" type="number" step="0.01" min="0.01" value="${esc(q?.unitPrice||'')}" required /></label><label><span>${esc(t('currency'))}</span><select name="currency">${currencies.map(code=>`<option ${String(q?.currency||replacementCurrency)===code?'selected':''}>${esc(code)}</option>`).join('')}</select></label></div><div class="form-two"><label><span>${esc(t('moq'))}</span><input name="moq" type="number" min="1" ${moqMax?`max="${esc(moqMax)}"`:''} value="${esc(q?.moq||'')}" required /></label><label><span>${esc(t('leadTime'))}</span><input name="leadTime" type="number" min="1" value="${esc(q?.leadTime||'')}" required /></label></div><label><span>${esc(t('sampleCost'))}</span><input name="sampleCost" value="${esc(q?.sampleCost||'')}" maxlength="10000" /></label><label><span>${esc(t('notes'))}</span><textarea name="notes" maxlength="10000">${esc(q?.notes||'')}</textarea></label>${fileField('quoteFiles')}<p class="form-message" id="quoteFormMessage"></p><button class="primary-btn" type="submit">${esc(q?t('save'):t('submit'))}</button></form>`;
}
...
async function handleAction(target){
  ...
  if(target.dataset.supplyProduct)return offerSupply(target.dataset.supplyProduct);
  ...
  if(target.dataset.approveCartReplacement){const r=(platformState.requests||[]).find(x=>x.id===target.dataset.requestId);if(!r)return;target.disabled=true;try{await mutate('requests',r.id,r.version,{approveCartReplacementQuoteId:target.dataset.approveCartReplacement});await loadData({render:false});closeModal();renderScreen();showToast(tr('تم اعتماد السعر والعرض الجديد وإرسال الطلب للمورد البديل.','New price and quote approved; the order was sent to the replacement supplier.'));}catch(error){showToast(error.message);target.disabled=false;}return;}
  if(target.dataset.rejectCartReplacement){const r=(platformState.requests||[]).find(x=>x.id===target.dataset.requestId);if(!r)return;target.disabled=true;try{await mutate('requests',r.id,r.version,{rejectCartReplacementQuoteId:target.dataset.rejectCartReplacement});await loadData({render:false});closeModal();renderScreen();showToast(tr('تم رفض العرض البديل. ستبحث الإدارة عن مورد آخر.','Replacement quote rejected. Admin can choose another supplier.'));}catch(error){showToast(error.message);target.disabled=false;}return;}
  if(target.dataset.approveReplacementQuote){const r=(platformState.requests||[]).find(x=>x.id===target.dataset.requestId);if(!r)return;target.disabled=true;try{await mutate('requests',r.id,r.version,{approveReplacementQuoteId:target.dataset.approveReplacementQuote});await loadData({render:false});closeModal();renderScreen();showToast(tr('تم اعتماد العرض البديل.','Replacement quote approved.'));}catch(error){showToast(error.message);target.disabled=false;}return;}
  if(target.dataset.rejectReplacementQuote){const r=(platformState.requests||[]).find(x=>x.id===target.dataset.requestId);if(!r)return;target.disabled=true;try{await mutate('requests',r.id,r.version,{rejectReplacementQuoteId:target.dataset.rejectReplacementQuote});await loadData({render:false});closeModal();renderScreen();showToast(tr('تم رفض العرض البديل. ستبحث الإدارة عن مورد آخر.','Replacement quote rejected. Admin can choose another supplier.'));}catch(error){showToast(error.message);target.disabled=false;}return;}
