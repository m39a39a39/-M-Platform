...
function renderHome(){
  ...
  if(role==='client'){
    ...
    $('screen').innerHTML=
      ...
      `<section class="ready-products-section"><div class="section-title ready-products-title"><div><h2>${esc(t('readyProducts'))}</h2><p>${esc(t('readyProductsSubtitle'))}</p></div></div><div class="public-offers-grid">${offers.map(publicOfferCard).join('')||empty()}</div></section>`;
