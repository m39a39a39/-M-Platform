...
function renderHome(){
  ...
  if(role==='client'){
    ...
    $('screen').innerHTML=
      ...
      `<section class="ready-products-section"><div class="section-title ready-products-title"><div><h2>${esc(t('readyProducts'))}</h2><p>${esc(t('readyProductsSubtitle'))}</p></div></div><div class="public-offers-grid">${pageOffers.map(publicOfferCard).join('')||empty()}</div>${productPagination(readyProductsPage,totalPages)}</section>`+
      companyFooterCard();
...
async function handleAction(target){
  ...
  if(target.dataset.action==='ready-products-next'){const total=Math.max(1,Math.ceil(((platformState?.publicOffers||[]).filter(o=>o.status==='published').length)/PAGE_SIZE));if(readyProductsPage<total){readyProductsPage++;renderHome();$('screen').scrollTop=0;}return;}
