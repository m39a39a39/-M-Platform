...
function cartReplacementSupplierPanel(x){
  ...
  return declined.map(child=>{
    ...
    const rejected=new Set(Array.isArray(x.cartReplacementRejectedQuoteIds)?x.cartReplacementRejectedQuoteIds:[]);
    const candidates=quotes.filter(q=>q.requestId===x.id&&q.status==='published'&&!rejected.has(q.id)&&q.supplierOrderStatus!=='cannot_fulfill'&&(q.replacementInterestId===child.id||(!q.replacementInterestId&&declined.length===1)));
    ...
    const busyElsewhere=new Set(invites.filter(v=>v?.interestId!==child.id&&!['selected','cancelled'].includes(v.status)).map(v=>v.supplierId));
    const newSuppliers=can('publish')?(state?.accounts||[]).filter(a=>a.role==='supplier'&&!a.blockedAt&&!a.deletedAt&&a.id!==oldSupplier&&!invited.has(a.id)&&!busyElsewhere.has(a.id)):[];
...
function replacementSupplierPanel(x,kind){
  ...
  const rejected=new Set(Array.isArray(x.replacementRejectedQuoteIds)?x.replacementRejectedQuoteIds:[]);
  const alternatives=(state?.quotes||[]).filter(q=>q.requestId===x.id&&q.status==='published'&&!rejected.has(q.id)&&q.id!==x.selectedQuoteId&&q.supplierOrderStatus!=='cannot_fulfill');
  const invited=new Set(Array.isArray(x.supplierIds)?x.supplierIds:[]);
  const newSuppliers=can('publish')?(state?.accounts||[]).filter(a=>a.role==='supplier'&&!a.blockedAt&&!a.deletedAt&&!invited.has(a.id)):[];
...
async function submitReplacementInvite(form){const x=(state?.requests||[]).find(v=>v.id===form.dataset.requestId);if(!x)return;const supplierId=String(form.supplierId.value||'');if(!supplierId)return;const supplierIds=[...new Set([...(Array.isArray(x.supplierIds)?x.supplierIds:[]),supplierId])];try{await mutate('requests',x,{supplierIds},true);closeModal();schedule();toast(tr('تم إرسال نفس طلب عرض السعر إلى المورد الجديد.','The same RFQ was sent to the new supplier.'));}catch(e){toast(e.message);}}
...
function cartOrderAdminPanel(request){
  ...
  const rows=snapshots.map((item,index)=>{
    const child=children.find(i=>i.id===item.interestId),offer=(state?.publicOffers||[]).find(o=>o.id===item.offerId),supplier=child?.assignedSupplierId?account(child.assignedSupplierId):(offer?account(offer.supplierId):null),statusValue=child?.supplierOrderStatus||'pending_confirmation';
