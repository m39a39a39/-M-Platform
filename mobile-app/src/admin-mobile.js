...
function offers(){
  const qs=(state?.quotes||[]).filter(x=>!x.deletedAt).map(x=>({...x,__kind:'quote'})),po=(state?.publicOffers||[]).filter(x=>!x.deletedAt).map(x=>({...x,__kind:'public'})),ints=(state?.interests||[]).filter(activeInterest);
