...
export async function media(user,id,res){
  ...
  let permitted=user?.id===m.owner_id;
  if(!permitted){
    const s=await snapshot(user),src=`/api/media/${id}`;
  ...
  assert(r.ok,502);res.setHeader('Content-Type',m.mime);res.setHeader('Cache-Control','private, no-store');res.end(Buffer.from(await r.arrayBuffer()));
}
