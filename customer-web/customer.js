document.addEventListener('m:ready', async () => {
  ...
  function statusMatch(r){
    ...
    return group===statusFilter||raw===statusFilter;
  }
  ...
  function render() {
    ...
    if(view==='market') {
      ...
      if(!s.publicOffers.filter(W.available).some(o=>search.match(o,[W.ref(o),W.copy(o,'title')])))html=W.empty();
    } else if(view==='interests') {
      html=s.interests.filter(i=>i.customerId===user.id).filter(i=>{const o=s.publicOffers.find(o=>o.id===i.offerId&&R.offerOpen(o,s));return search.match(i,[o?W.ref(o):'',o?W.copy(o,'title'):'']);}).map(i=>{
  ...
  document.addEventListener('click', async e =>{
    ...
    if(select) {
      ...
      r.selectedQuoteId=q.id;await M.save(s);document.getElementById('workflowDialog')?.remove();go('selected');return;
    }
